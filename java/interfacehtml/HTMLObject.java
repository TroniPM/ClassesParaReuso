...
public class HTMLObject {
	...
	public static HTMLObject parse(String html) {
		...
		for(HTMLObject in: allObjects) {
			...
			String aux;
			if(in.isInlineTag()) {
				aux = in.getStr_start().replace(TAG_START+in.getTag(), "").replaceAll(TAG_FINISH_INLINE, "").trim();
			} else {
				aux = in.getStr_start().replaceAll(TAG_START+in.getTag(), "").replaceAll(TAG_END, "").trim();
			}
			
			//split por =" AND ='
			String[] att = aux.split("=\"");
			for(String in1 : att) {
	...
	public boolean equals(Object obj) {
		...
		//		System.out.println(this + " == " + o + " || " + this.getStr_start() + " == " + o.getStr_start());
